...
app.get("/urls", (req, res) => {
  if (!req.session["user_id"]) {
    res
      .status(400)
      .send(
        `You are not logged in. Please <a href="/login">log in</a> or <a href="/register">register</a>.`
      );
  }
  const templateVars = {
...
app.post("/urls", (req, res) => {
  if (!req.session["user_id"]) {
    res
      .status(401)
      .send("Please log in or register in order to create new short links.");
  }
...
app.post("/urls/:shortURL", (req, res) => {
  ...
  if (
    !urlsForUser(req.session["user_id"]).hasOwnProperty(req.params.shortURL)
  ) {
...
app.post("/login", (req, res) => {
  ...
  if (
    ...
    !bcrypt.compareSync(req.body.password, matchingUser.password)
  ) {
